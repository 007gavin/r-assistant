...
#' Uses async API calls for responsive UI.
...
addin_chat <- function() {
  ...
  if (ready && rstudioapi::isAvailable()) {
    # Use sendToConsole to force RStudio to execute viewer in its own context
    viewer_cmd <- sprintf('rstudioapi::viewer("%s")', url)
    rstudioapi::sendToConsole(viewer_cmd, execute = TRUE)
  } else if (ready) {
    browseURL(url)
  }

  message("[R Assistant] Chat running at ", url)
...
.build_chat_server <- function(init_sel = "") {
  function(input, output, session) {
    ...
    do_send <- function() {
      ...
      # Run API call in background future (non-blocking)
      future::future({
    ...
    shiny::observeEvent(input$ra_model_change, {
      ...
      if (!is.null(new_model) && nzchar(new_model)) {
        ...
        message("[R Assistant] Model: ", new_model)
      }
    })
...
.html_model_options <- function(provider, current_model) {
  ...
  if (length(models) == 0) {
    return(shiny::HTML(paste0('<option value="', current_model, '">', current_model, '</option>')))
  }
...
.html_history_panel <- function() {
  ...
  items <- vapply(hist, function(msg) {
    ...
    paste0('<div class="ra-history-item">',
           '<span class="ra-hist-role ', role_cls, '">', toupper(role), '</span>',
  ...
  shiny::HTML(paste0(
    '<div class="ra-history-overlay" id="ra-history-panel">',
    '<div class="ra-history-header">',
    '<h4>History (', length(hist), ' messages)</h4>',
    '<button class="ra-history-close" onclick="document.getElementById(\'ra-history-panel\').remove()">&times;</button>',
    '</div>',
...
addin_explain <- function() {
  ...
  if (rstudioapi::isAvailable()) {
    tryCatch({ .show_result_viewer("Code Explanation", result) }, error = function(e) {})
  }
}
...
addin_fix <- function() {
  ...
  if (rstudioapi::isAvailable()) {
    tryCatch({ ctx <- rstudioapi::getActiveDocumentContext(); sel <- ctx$selection[[1]]$text; if (nzchar(trimws(sel))) code <- sel }, error = function(e) {})
    tryCatch({ error <- geterrmessage(); if (!nzchar(error)) error <- NULL }, error = function(e) {})
  }
...
.offer_code_insertion <- function(response, insert_before = FALSE) {
  ...
  choice <- utils::menu(
    ...
    choices = c("Replace selected code with first block", "Insert code after selection",
                "Copy to clipboard", "Do nothing (just show)"))
  switch(choice,
    { rstudioapi::insertText(code_blocks[[1]]); message("Code replaced.") },
    { ctx <- rstudioapi::getActiveDocumentContext(); rstudioapi::insertText(ctx$selection[[1]]$range$end, paste0("\n\n", code_blocks[[1]])); message("Code inserted.") },
...
.chat_css <- function() {
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ra-bubble pre { background: #11111b; padding: 10px 12px; border-radius: 8px; margin: 8px 0; overflow-x: auto; border: 1px solid #313244; }
...
.chat_html <- function(model, init_sel, provider = "deepseek") {
  ...
  <div class="ra-wrap">
    <div class="ra-header">
      ...
      <div class="ra-header-right">
        <button class="ra-hbtn" id="ra_btn_history" title="History"
        ...
        <button class="ra-hbtn" id="ra_btn_new" title="New chat"
        ...
        <button class="ra-hbtn" id="ra_btn_insert" title="Insert code"
    ...
    <div class="ra-input-wrap">
      <div class="ra-input-box">
        <textarea id="ra_input" rows="1" placeholder="Ask R Assistant..."></textarea>
        <button class="ra-send-btn" id="ra_send" onclick="Shiny.setInputValue(\'ra_send\', Math.random())">
          <svg viewBox="0 0 16 16" fill="currentColor">
            <path d="M8 1.5a.5.5 0 01.5.5v10.793l3.146-3.147a.5.5 0 01.708.708l-4 4a.5.5 0 01-.708 0l-4-4a.5.5 0 01.708-.708L7.5 12.793V2a.5.5 0 01.5-.5z" transform="rotate(-90 8 8)"/>
          </svg>
        </button>
      </div>
      <div class="ra-input-footer">
        ...
        <div class="ra-model-badge">
          <span class="ra-model-dot"></span>', model, '
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    $(document).on("keydown", "#ra_input", function(e) {
      if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); Shiny.setInputValue("ra_send", Math.random()); }
    });
